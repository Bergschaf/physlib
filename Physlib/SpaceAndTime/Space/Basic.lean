...
instance {d : ℕ} : IsManifold (𝓡 d) ⊤ (Space d) where
  compatible := by
    ...
    have h :
        (homEuclideanSpace d).toOpenPartialHomeomorph.symm ≫ₕ
        (homEuclideanSpace d).toOpenPartialHomeomorph =
        OpenPartialHomeomorph.ofSet Set.univ (by simp) := by
      ext v i <;> simp [OpenPartialHomeomorph.trans_apply]
    rw [h]
    exact ofSet_mem_contDiffGroupoid (by simp)
